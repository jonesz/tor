 ...
 * TODO @jones: Our offline key component isn't building an OPE cipher
 * thing-a-majig at key creation time; as an OPE cipher is linked to the
 * blinded secret key, we realistically need to do this.
 * */
static crypto_ope_t *
                        ...
                        hs_service_descriptor_t *desc)
{
  ...
  /* TODO @jones: OPE cipher needs to be loaded/created aswell. More information
   * is included in the "generate_ope_cipher_for_desc" comment. */
  if (config->offline_keys) {
    /* Attempt to load a blinded public key, a descriptor keypair, and a
     * cert. */
    if (load_offline_keys(service, desc, time_period_num) < 0) {
