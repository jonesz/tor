...
/* This function declaration has been changed to x_i, to prevent collisions with the tor code.
 * XXX: We should just be using the keccak stuff already in src/ext. */
void shake128_i(unsigned char *output, unsigned long long outlen, const unsigned char *input,  unsigned long long inlen)
{
  uint64_t s[25] = {0};
...
/* This function declaration has been changed to x_i, to prevent collisions with the tor code.
 * XXX: We should just be using the keccak stuff already in src/ext. */
void shake256_i(unsigned char *output, unsigned long long outlen, const unsigned char *input,  unsigned long long inlen)
{
  uint64_t s[25];
